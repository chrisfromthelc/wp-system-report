	...
	if [[ $WP_VERSION == 'nightly' || $WP_VERSION == 'trunk' ]]; then
		...
		if [ $WP_VERSION == 'latest' ]; then
			...
			if [[ $WP_VERSION =~ [0-9]+\.[0-9]+\.[0] ]]; then
				...
				LATEST_VERSION=$(grep -o '"version":"'$VERSION_ESCAPED'[^"]*' $TMPDIR/wp-latest.json | head -1 | sed 's/"version":"//')
			fi
			if [[ -z "$LATEST_VERSION" ]]; then
...
install_test_suite() {
	# portable in-place argument for both GNU sed and Mac OS DMG://daringfireball.net/2007/01/bbedit_text_filters sed
	local ioption='-i'
